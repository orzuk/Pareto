...
# Comptue Probability that X_1, X_2 not dominated by X_3,...,Xn (they can still dominate each other), with large integers.
def pareto_P_Bj1c_and_Bj2c_python(k, n, dps = 100):
    start = time.time()
    mp.dps = dps  # set significant digits
    n_factorial_vec = [math.factorial(i) for i in range(n-1)]  # prepare in advance
    max_denom_lcm = lcm_first(n)**(3*k)
    ...
    for a in range(n-1):
        ...
        for b in range(n-1-a):
            for c in range(n-1-a-b):
                d = n-2-a-b-c
                numerator = (n_factorial_vec[n-2] // (n_factorial_vec[a]*n_factorial_vec[b]*n_factorial_vec[c]*n_factorial_vec[d])) * (-1)**(a+b) * (a+b+2*c+2)**k
                denominator = ((a+c+1)*(b+c+1)*(a+b+c+2))**k
                e_k_n_int += numerator * (max_denom_lcm // denominator)
                ctr += 1
#    return 0.1
